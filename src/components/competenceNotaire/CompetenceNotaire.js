...
const CompetenceNotaire = () => {
      ...
      if (stringInArray(departementBien, departementLimitropheParis)) {
        if (stringInArray(departementNotaire, departementLimitropheParis)) {
          if (notaireAcquereur === "true") {
            ...
            return (
              <div>
                Nous constituons le dossier et le confrère tient la plume
              </div>
            );
        ...
        if (notaireAcquereur === "true") {
          return (
            <div>
              ...
              constitution et la plume nous reviens
            </div>
          );
        } else
  ...
  const notaireRadioButton = () => {
    return (
      <>
        <label>Sommes nous notaire acquéreur</label>
        <input
          type="radio"
          name="notaireAcquereur"
          value={true}
  ...
  const departementForm = (objectValue) => {
    return (
      <form name="form1" method="post" action="" onChange={handleChange}>
  ...
  return (
    <>
      <div>{notaireRadioButton()}</div>
      <label>Sélectionner le département du notaire participant :</label>
      <div>{departementForm("departementNotaire")}</div>
      <label>
        Sélectrionner le département dans lequel le bien vendu est situé :
      </label>
      <div>{departementForm("departementBien")}</div>
      <div>{competenceLogic()}</div>
    </>
  );
};

export default CompetenceNotaire;
