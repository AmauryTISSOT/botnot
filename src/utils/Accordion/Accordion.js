...
const Accordion = ({ dataObject, rootPath }) => {
  ...
  return (
    <div className="flex flex-col cursor-pointer bg-blue-100 w-150 rounded">
      {dataObject.map(({ title, svg, content }, index) => (
        <React.Fragment key={index}>
          <div
            className={`w-full h-full border-slate-100 text-slate-700 rounded-t-1 group cursor-pointer items-center border-b border-solid p-4 text-left font-semibold flex justify-between hover:bg-blue-200 ${
              isActive[title] && "bg-blue-200"
            }`}
