...
describe("CustomerQuizSummary component test", () => {
    ...
    test("Should correctly render several answer", () => {
        ...
        const answerElement2 = screen.getByText("Il existe une association syndicale")
        const answerElement3 = screen.queryByText("Le bien dépend d'une copropriété horizontale")
    ...
    test("Should correctly render the text area", () => {
        const mockDataState = {situationTerrainLotissement : "textArea"}
        render(<CustomerQuizSummary dataState={mockDataState}/>)
        const answerElement = screen.queryByText("textArea")
        expect(answerElement).toBeInTheDocument();
    })
})